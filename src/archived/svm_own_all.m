function svm_own_all ( num_iterations )
    ...
    % load train datasets
    real = loadset('mirbase20-nr','all', 0); % 13070 all-sp entries
    pseudo1 = loadset('coding','all', 1);    % 8494 hsa-only dataset
    pseudo1 = shuffle(pseudo1);
    pseudo2 = loadset('other-ncrna','all', 2); % 129  hsa-only dataset
    pseudo3 = loadset('functional-ncrna','all', 3); % 2657  hsa-only dataset
    pseudo  = shuffle([ pseudo1(1:2401,:); pseudo2 ]);
    ...
    for r=1:4
      ...
      [tr_real ts_real]     = partset(  real,  n_real_train);
      [tr_pseudo ts_pseudo] = partset(pseudo,n_pseudo_train);
	  ...
	  if ir > size(tr_real,2)
              [tr_real ts_real] = partset(real,n_real_train); ir=1;
	  end
	  if ip > size(tr_pseudo,2)
              [tr_pseudo ts_pseudo] = partset(pseudo,n_pseudo_train); ip=1;
	  end
